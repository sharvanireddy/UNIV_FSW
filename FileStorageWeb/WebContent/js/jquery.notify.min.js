	...
	e.cNotify = function(t) {
		var n = {
			...
			_hide : function(n) {
				...
				if (!n) {
					...
					setTimeout(
							...
							typeof t.delay != "number" || t.delay == "auto" ? u < 2500 ? 2500
									: u
									: t.delay)
				}
			},
			_setContent : function() {
				...
				if (e
						.inArray(t.type, [ "success", "error", "warning",
								"info" ]) != -1) {
					r.addClass(t.type)
				}
				if (t.title && t.title != null) {
					r.find(t._classes.content).prepend(
							"<h3>" + t.title + "</h3>")
				} else {
					r.addClass("notify-without-title")
				}
				if (t.message && t.message != null) {
					r.find(t._classes.content).append(
							"<p>" + t.message + "</p>")
				}
				...
				if (t.icon && t.icon != null) {
					var n = e(t.icon).is("img") ? e(t.icon) : e(t.icon).find(
							"img").size() != 0 ? e(t.icon).find("img")
							: e(t.icon);
					r
							.prepend('<div class="notify-icon"><div class="notify-icon-inner">'
									+ t.icon + "</div>")
				} else {
					...
					if (e("body").find(t._classes.overlay).size() == 0) {
						e(
								'<div class="notify-overlay'
										+ (typeof t.overlay == "string" ? " "
												+ t.overlay : "") + '"></div>')
								.hide().appendTo("body").fadeIn(250)
					}
					r.addClass("notify-overlayed")
				}
				if (t.closeBtn) {
					r
							.prepend('<a href="javascript:;" class="notify-close-btn"></a>');
				...
				if (n) {
					n.load(function() {
						var e = Math.round(r.find(t._classes.iconI)
								.outerHeight() / 2);
						if (e > 0) {
			...
			_poisition : function() {
				var n = "notify-";
				if (t.position && t.position != null
						&& e.isPlainObject(t.position) && t.position.x
						&& t.position.y) {
					switch (t.position.y) {
					case "top":
						n += "top-";
				...
				if (i > 1) {
					...
					switch (pS[1]) {
					case "top":
						o.top = parseFloat(e("body").find(".notify." + n).eq(
								i - 2).css("top"))
								+ e("body").find(".notify." + n).eq(i - 2)
										.outerHeight() + t.spacing;
						break;
					case "bottom":
						o.bottom = parseFloat(e("body").find(".notify." + n)
								.eq(i - 2).css("bottom"))
								+ e("body").find(".notify." + n).eq(i - 2)
										.outerHeight() + t.spacing;
						break
